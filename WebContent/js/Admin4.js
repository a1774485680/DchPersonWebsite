	...
	function page(data){
		...
		var arrspan=["span1","span2","span3","span4","span5","span6","button"];
		var css=data;
		...
		var fal=0;
		 ...
		 $.each(arr, function(){     
			    ...
			    if(fal<line){
					...
					$("#"+i+"-span"+arrspan[6]).html("状态"+Jsondata[fal].publish);
					$("#"+i+"-button").html("切换状态");
	...
	function clickButton(as){
		
		console.log(as);
		var author="div"+as+"-spanspan1";
		var titile="div"+as+"-a";
		var getauthor= $("#"+author).html();
		var gettitile=$("#"+titile).html();
	...
	$(function (){
		...
		$.post("/MyFirstWeb/AdminCotroller/passageLoad.do",function(data){
			var fal=1;//记录是第几个div
		...
		//设置分页的系统 获得当前的行数
		$.post("/MyFirstWeb/AdminCotroller/allPage.do",{NowCategory:Nowclassify},function(data){
			allpage=data.count;
		...
		$("#Bgo").click(function(){
			...
			$.post("/MyFirstWeb/AdminCotroller/goPage.do",{goPage:Npage,NowCategory:Nowclassify},function(data){
				page(data);
			},"text");
			}else{
				$("#Tpage").val("1");
		...
		$("#BupPage").click(function(){
			...
			if(up>0){
				$.post("/MyFirstWeb/AdminCotroller/upPage.do",{upPage:up,NowCategory:Nowclassify},function(data){
					page(data);
				},"text");
				$("#Tpage").val(up);
			}
		...
		$("#BdownPage").click(function(){
			...
			if(down<=allpage){
				$.post("/MyFirstWeb/AdminCotroller/downPage.do",{downPage:down,NowCategory:Nowclassify},function(data){
					page(data);
				},"text");
				$("#Tpage").val(down);				
			} 
	...
	function classify(id)
		{		
				...
				falof2=1;//将获得类别文章信息的标志位设置位1，即不允许点击类别
		 		...
		 		$.post("/MyFirstWeb/AdminCotroller/classify.do",{NowCategory:Nowclassify},function(data){
		 			page(data);
		 			falof2=0;
		 			if(falof1==0&&falof2==0){			 			
			 			$.each(arr,function(){
